...
class Solver(object):
    ...
    def __init__(self, alphas, betas, weights, constraints):
        ...
        '''
        Changing the way that we handle the constraints matrix here.
        ...
        '''
        for rev_idx, row in enumerate(constraints):
            for paper_idx, col in enumerate(row):
                constraint_value = constraints[rev_idx, paper_idx]
                # these values are different because the constraint matrix represents
                # hard positive constraints as 1 and hard negative constraints as -1,
                # but the gurobi constraints set hard negative constraints as 0.
                if constraint_value == -1:
                    self.constraints.append((rev_idx, paper_idx, 0))
                elif constraint_value == 1:
                    self.constraints.append((rev_idx, paper_idx, 1))
                else:
                    self.constraints.append((rev_idx, paper_idx, -1))
        '''
        End of modification.
    ...
    def add_hard_consts(self, constrs, log_file=None):
        ...
        Value of 1 means that reviewer *must* review paper
        Value of 0 means that reviewer *cannot* review paper
        Value
        """
