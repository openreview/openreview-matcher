...
class MaximinAssignmentBuilder(GraphBuilder):
    '''
    Implements an objective that maximizes the minimum scoring papers
    '''
    def _sorted_pairs(self, assignment_graph, sim_matrix):
    ...
    def _check_maxflow(self, sim_matrix, current_pairs, assignment_graph):
        '''
    ...
    def _binsearch_for_parameters(self, sim_matrix, assignment_graph):
        '''
        ...
        '''
        sorted_pairs = self._sorted_pairs(assignment_graph, sim_matrix)
        ...
        while lower_bound < upper_bound:
            ...
            maxflow, start_nodes, end_nodes, capacities, costs = self._check_maxflow(sim_matrix, current_pairs, assignment_graph)
    ...
    def build(self, graph):
        ...
        sim_matrix = -1 * graph.cost_matrix
        # each sorted pair has the following form: [reviewer_index, paper_index]
        start_nodes, end_nodes, capacities, costs = self._binsearch_for_parameters(sim_matrix, graph)
