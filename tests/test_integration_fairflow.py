...
def test_integration_emergency_review(openreview_context):
    '''
    Basic integration test for emergency reviews. Makes use of the OpenReview Builder
    '''
    openreview_client = openreview_context['openreview_client']
    test_client = openreview_context['test_client']

    conference_id = 'AKBC.ws/1999/Conference'
    num_reviewers = 10
    num_papers = 10
    reviews_per_paper = 3
    max_papers = 5
    ...
    deployed_results = conference.set_assignments(assignment_title='integration-test-new')
    
    time.sleep(5)
    
    print('deployed res::', deployed_results)
    assert deployed_results
    
    assert openreview_client.get_group('AKBC.ws/1999/Conference/Paper1/AnonReviewer1')
